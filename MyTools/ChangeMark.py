# --------------------------------------------------------#
#   该文件用于调整mark的格式，变为由原来的[0, 255]变为[0, 1]
# --------------------------------------------------------#
import os
...
# -----------------------------------------------------------------------------------#
Origin_path = r"E:\郑的江山社稷-backup\甲状腺结节\dataset\2023Dataset7\Dataset7由1+4合成的大型甲状腺肿瘤超声图像分割数据集（分割 - [0,1]格式）\test\masks"
Out_path = r"E:\郑的江山社稷-backup\甲状腺结节\dataset\2023Dataset7\Dataset7由1+4合成的大型甲状腺肿瘤超声图像分割数据集（分割 - [0,1]格式）\test\masks0"
...
if __name__ == "__main__":
    ...
    #   遍历标签并赋值
    # ---------------------------#
    png_names = os.listdir(Origin_path)
    print("正在遍历全部标签。")
    for png_name in tqdm(png_names):
        png = Image.open(os.path.join(Origin_path, png_name))
        w, h = png.size
        ...
        out_png.save(os.path.join(Out_path, png_name))
    ...
    for png_name in tqdm(png_names):
        png_file_name = os.path.join(Out_path, png_name)
